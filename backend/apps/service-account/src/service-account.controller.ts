...
import { IUserSearchManyByIdsResponse } from './interfaces/user-search-by-ids.interfaces';
...
export class ServiceAccountController {
  ...
  //Get users by ids, also useful with matching each vehicule with its users
  /*@MessagePattern('users_get_by_ids')
  public async getUserByIds(ids: string[]): Promise<IUserSearchManyByIdsResponse> {
    let result: IUserSearchManyByIdsResponse;

    if (ids) {
      const users = await this.userService.searchUsersByIds(ids);
      if (users) {
        result = {
          status: HttpStatus.OK,
          message: 'user_get_by_id_success',
